  ...
  <div class="addEdit-block">
    ...
    >
      <el-row>
          ...
          <el-form-item class="input" label="垃圾名称" prop="lajimingcheng">
              <el-input v-model="ruleForm.lajimingcheng" 
                placeholder="垃圾名称" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="垃圾类型" prop="lajileixing">
          <el-select v-model="ruleForm.lajileixing" placeholder="请选择垃圾类型">
            <el-option
                v-for="(item,index) in lajileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="垃圾类型" prop="lajileixing">
	      <el-input v-model="ruleForm.lajileixing"
                placeholder="垃圾类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.tupu" label="图谱" prop="tupu">
          <file-upload
          tip="点击上传图谱"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.tupu?ruleForm.tupu:''"
          @change="tupuUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.tupu" label="图谱" prop="tupu">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.tupu.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
                                                            </el-row>
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.lajimingcheng = true;
            continue;
	            	            ...
	            	            if(o=='lajileixing'){
            this.ruleForm.lajileixing = obj[o];
	    this.ro.lajileixing = true;
            continue;
          }
	            	            if(o=='tupu'){
            this.ruleForm.tupu = obj[o];
	    this.ro.tupu = true;
            continue;
          }
	            	            if(o=='xiangqing'){
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `lajitupu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.lajitupuCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    back() {
      ...
      this.parent.contentStyleChange();
                                                ...
                                                tupuUploadChange(fileUrls) {
                this.ruleForm.tupu = fileUrls;
				this.addEditUploadStyleChange()
            },
                        	addEditStyleChange() {
	  this.$nextTick(()=>{
